...
export default async function AirsoftGameTactics({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const content = (t.raw(`Posts.airsoft-game-tactics.content`) as unknown) as Content;
    return (
      ...
      />
      <h1 className="text-4xl md:text-5xl font-bold text-center">{t("Posts.airsoft-game-tactics.title")}</h1>
      <p className="text-center text-muted-foreground text-lg md:text-xl mt-2 mb-8">{t("Posts.airsoft-game-tactics.description")}</p>
